...
// Function that updates the sum value based on resolved promise and synchronous operation
export const updateSumValue = async (inputPromise) => {
  let sum = 2;

  // Await the result of the promise that is passed in as an argument
  const promiseValue = await inputPromise;

  // Update the sum with the promise value
  sum += promiseValue;

  // Return 10 regardless of the sum
  return 10;
};
